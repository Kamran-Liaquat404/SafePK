...
function openScamModal(id) {
  // If on learn.html and showDedicatedArticle exists, open dedicated article view directly
  if (typeof window.showDedicatedArticle === 'function') {
    window.showDedicatedArticle(id, true);
    return;
  }

  // If on another page, navigate to dedicated article page on learn.html
  const isSubpage = window.location.pathname.includes('/pages/') || window.location.href.includes('/pages/');
  const targetLearnUrl = isSubpage ? `learn.html?article=${encodeURIComponent(id)}` : `pages/learn.html?article=${encodeURIComponent(id)}`;
  window.location.href = targetLearnUrl;
}
...
function initSecurityTools() {
  const toolsOverviewGrid = document.getElementById('tools-overview-grid');
  const toolPanels = document.querySelectorAll('.tool-panel');
  if (!toolsOverviewGrid && toolPanels.length === 0) return; // Skip if not on tools page
  ...
  // 2. TOOL VIEW NAVIGATION SYSTEM (Grid Overview vs Dedicated Tool View)
  function showToolView(toolId) {
    const gridEl = document.getElementById('tools-overview-grid');
    const heroSec = document.getElementById('tools-sec-title-container');

    if (!toolId || toolId === 'grid' || toolId === 'all') {
      // Show Grid Overview
      ...
      try {
        ...
        console.warn("[SafePK] History pushState skipped:", err);
      }
      if (window.lucide) {
        window.lucide.createIcons();
      }
    ...
    // Hide Grid Overview & Hero, Show Selected Panel
  ...
  // Bind click handlers to tool grid cards
  document.querySelectorAll('[data-open-tool]').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.preventDefault();
      const toolId = btn.getAttribute('data-open-tool');
      if (toolId) showToolView(toolId);
    });
  });

  // Bind click handlers to back buttons
  ...
  window.activateToolTab = showToolView;
  window.showToolView = showToolView;
  ...
  try {
    ...
    activeTool = urlParams.get('tool') || urlParams.get('tab');
    scamIdParam = urlParams.get('scamId');
  } catch (err) {
    console.warn("[SafePK] Deep-linking parameters read restricted:", err);
  }

  if (activeTool && document.getElementById(`panel-${activeTool}`)) {
    showToolView(activeTool);
  } else {
    showToolView('grid'); // Default view is the responsive Grid Overview
  }
  ...
  if (fiaBuilderForm) {
    fiaBuilderForm.addEventListener('submit', (e) => {
      ...
      if (fiaDraftPre) {
        ...
        if (badge) {
          badge.textContent = "Report Generated ✓";
          badge.style.background = "rgba(16, 185, 129, 0.15)";
          badge.style.color = "var(--color-success)";
        }

        // Scroll into view on mobile devices for ease of access
        if (window.innerWidth < 992) {
          fiaDraftPre.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }
      }
    });
  }
